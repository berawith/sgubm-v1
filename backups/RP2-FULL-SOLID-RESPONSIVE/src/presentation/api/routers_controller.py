...
def get_routers():
    ...
    for router in routers:
        ...
        # Contar clientes registrados en BD para este router
        all_clients = client_repo.get_all()
        clients_for_router = [c for c in all_clients if c.router_id == router.id]
...
def sync_router(router_id):
    ...
    try:
        ...
        def is_ip_allowed(ip_str):
            if not has_segments_filter: return False
        ...
        try:
            ...
            logger.info(f"Sync sync_router: Scanning router sources. ppp_active={len(ppp_active)}, queues={len(existing_queues)}")
            ...
            # 3. Descubrir Candidatos (DHCP/ARP)
            dhcp_leases = adapter.get_dhcp_leases()
            arp_table = adapter.get_arp_table()
            
            logger.info(f"Sync sync_router: Candidates found. dhcp={len(dhcp_leases)}, arp={len(arp_table)}")
            
            candidates = {} # ip -> { host, mac, source }
            
            for lease in dhcp_leases:
                ip = lease.get('address')
                if not ip or not is_ip_allowed(ip): continue
                if ip in seen_ips: continue
                # Priorizar hostname real
            ...
            for arp in arp_table:
                ip = arp.get('address')
                if not ip or not is_ip_allowed(ip): continue
                if ip in seen_ips or ip in candidates: continue
                candidates[ip] = {
            ...
            if confirm:
                for ip, info in candidates.items():
                    ...
                    while username.lower() in seen_mikrotik_usernames or username.lower() in existing_usernames:
                        username = f"{base_username} ({ip.split('.')[-1]})"
                        if counter > 1:
                            username = f"{base_username} ({ip.split('.')[-1]}-{counter})"
                        counter += 1
                    ...
                    try:
                        ...
                        # B. Crear Simple Queue en MikroTik (ESTANDAR WinBox 15M - Sin Comentario)
                        queue_payload = {
                            'queue_name': username,
                        ...
                        if result.get('success'):
                             ...
                             provisioned_count += 1
                             logger.info(f"Auto-provisioned client {username} at {ip}")
...
def setup_cutoff(router_id):
    """
    Configura las reglas de firewall para cortes (Address List CORTADOS-SGUB)
    """
    db = get_db()
    router_repo = db.get_router_repository()
    ...
    def fetch_router_metrics(router_data):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error in metrics fetch for router {router_id}: {e}")
                if adapter._is_connected:
                    adapter.disconnect()
                return {'id': router_id, 'status': 'offline', 'error': str(e)}
                
        except Exception as e:
            logger.error(f"Critical error in worker for router {router_id}: {e}")
            return {'id': router_id, 'status': 'offline', 'error': str(e)}

    # 1. Extraer datos necesarios antes de los hilos (Evita problemas de sesión SQL)
    routers_data = []
    for r in routers:
        routers_data.append({
            'id': r.id,
    ...
    for secret in pppoe_secrets:
        try:
            ...
            client_data = {
                ...
                'plan_name': f"{secret.get('download_speed', '')} / {secret.get('upload_speed', '')}",
