...
class RouterRepository:
    ...
    def get_by_status(self, status: str) -> List[Router]:
        ...
        return self.session.query(Router).filter(Router.status == RouterStatus(status)).all()
    ...
    def update_metrics(self, router_id: int, metrics: Dict[str, Any]) -> Optional[Router]:
        ...
        if router:
            ...
            router.status = RouterStatus(metrics.get('status', router.status.value))
...
class ClientRepository:
    ...
    def get_by_status(self, status: str) -> List[Client]:
        ...
        return self.session.query(Client).filter(Client.status == ClientStatus(status)).all()
    
    def search(self, query: str) -> List[Client]:
        """Busca clientes por nombre, código o documento"""
